...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches) {
        int packageDimensions = (lengthInches * widthInches * heightInches);

        return weightPounds <= MAX_WEIGHT_POUNDS && packageDimensions <= MAX_CUBIC_INCHES;
    }
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        int packageDimensions = (lengthInches * widthInches * heightInches);
        if (weightPounds <= MAX_WEIGHT_POUNDS && packageDimensions <= MAX_CUBIC_INCHES && lengthInches > MAX_DIMENSION_INCHES || widthInches > MAX_DIMENSION_INCHES || heightInches > MAX_DIMENSION_INCHES && isSurchargePaid) {
            return true;
        }
        else if (weightPounds <= MAX_WEIGHT_POUNDS && packageDimensions <= MAX_CUBIC_INCHES && lengthInches <= MAX_DIMENSION_INCHES && widthInches <= MAX_DIMENSION_INCHES && heightInches <= MAX_DIMENSION_INCHES) {
            return true;
        }
        else {
            return false;
