 ...
 * 
 * @param {array} sadArray is an array of strings to be prepended to
 * @returns {array} the resulting array
 */

function makeHappy(sadArray) {
     happyArray = sadArray.map(str => 'Happy ' + str);
 ...
 * 
 * @param {array} objectsArray is an array of objects
 * @return {array} is an array of strings  
 */

function getFullAddressesOfProperties(objectsArray) {
     objectsArray.map(({
          streetNumber, streetName, streetType, city, state, zip }) => {
...
function getSumOfSubArrayValues(array = []) {
     ...
     let fullSum = array.reduce(
          (sum, element) => {
               let arraySum = element.reduce(
                    (subSum, value) => {
                         return subSum + value;
                    }, 0);
